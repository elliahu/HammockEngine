...
void Hmck::KeyboardMovementController::moveInPlaneXZ(Window &window, float dt, std::shared_ptr<Entity> entity) {
    ...
    if (mouseInMotion) {
        ...
        entity->transform.rotation.y -= glm::clamp(glm::radians(static_cast<float>(offsetX)), -360.0f, 360.0f);
        entity->transform.rotation.x -= glm::clamp(glm::radians(static_cast<float>(offsetY)), -89.0f, 89.0f);
